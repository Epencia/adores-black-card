...
export default function ListeTransactionReglement({navigation,item,route}){
  ...
  useEffect(()=>{
    navigation.setOptions({title: 'Réglement des crédits'});
  ...
  const getTransaction = async () => {
    setIsLoading(true);
   try {
    const response = await fetch(`https://adores.tech/api/carte/transaction-reglement.php?matricule=${user[0].matricule}`, {
      headers: {
