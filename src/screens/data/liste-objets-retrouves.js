...
export default function ListeObjetsRetrouves ({navigation,item}){
...
const handleRefresh = () => {
 ...
 useEffect(()=>{
  navigation.setOptions({title: 'Appareils retrouvés'});
...
const getListeObjetsDisparus = async () => {
  setIsLoading(true);
 try {
  const response = await fetch(`https://adores.tech/api/carte/liste-objet-retrouves.php?matricule=${user[0].matricule}`, {
    headers: {
...
value={searchTerm}
 />
    ...
    <MaterialCommunityIcons color="green" name='phone' size={35} />
        ) : item.categorie_objet === 'Ordinateur' ? (
    <MaterialCommunityIcons color="green" name='laptop' size={35} />
       ) : item.categorie_objet === 'Moto' ? (
    <MaterialCommunityIcons color="green" name='motorbike' size={35} />
       ) : item.categorie_objet === "Piece d'identite" ? (
    <MaterialCommunityIcons color="green" name='file-outline' size={35} />
      ) : (
    <MaterialCommunityIcons color="green" name='fire-circle' size={35} />
       )
       }
            </View>
