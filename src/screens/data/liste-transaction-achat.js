...
export default function ListeTransactionAchat({navigation,item,route}){
  ...
  useEffect(()=>{
    navigation.setOptions({title: 'Résumé des achats'});
  ...
  const getTransaction = async () => {
    setIsLoading(true);
   try {
    const response = await fetch(`https://adores.tech/api/carte/transaction-achat.php?matricule=${user[0].matricule}`, {
      headers: {
