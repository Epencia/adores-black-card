...
export default function ListeTransactionVente({navigation,item,route}){
  ...
  useEffect(()=>{
    navigation.setOptions({title: 'Résumé des ventes'});
  ...
  const getTransaction = async () => {
    setIsLoading(true);
   try {
    const response = await fetch(`https://adores.tech/api/carte/transaction-vente.php?matricule=${user[0].matricule}`, {
      headers: {
